...
$_SESSION["sSQLFile"]="REP/REP0701.sql";  //SQL檔路徑
...
Case 'Select':
	...
	//父宣告
	$_SESSION["TitleTdName_REP0701"]=Array();
	$_SESSION["SQL_REP0701"]="";
	$_SESSION["Field_REP0701"]=Array();
	...
	if (Trim($ShowType)=="mon") {
		...
		for ($x=0; $x<$iDiff; $x++) {
			//上方Title
			$_SESSION["TitleTdName_REP0701"]=UnidimensionalArrayAppend($_SESSION["TitleTdName_REP0701"], Replace(Left(DateAdd("m", $x, $YearStart_01."/".$MonthStart_01."/01"), 7), "/", "-"));
			//SQL Script
			$_SESSION["SQL_REP0701"]=$_SESSION["SQL_REP0701"]."(select count(*) from orderdetail where orderdetail.pi01=productinfo.pi01 and (select oh26 from orderheader where orderheader.oh01=orderdetail.oh01) in ('退貨處理中', '已退貨', '已換貨', '已退款') and left(lasttime, 7)='".Replace(Left(DateAdd("m", $x, $YearStart_01."/".$MonthStart_01."/01"), 7), "/", "-")."' ) as '".Replace(Left(DateAdd("m", $x, $YearStart_01."/".$MonthStart_01."/01"), 7), "/", "-")."', ";
			//Field
			$_SESSION["Field_REP0701"]=UnidimensionalArrayAppend($_SESSION["Field_REP0701"], Replace(Left(DateAdd("m", $x, $YearStart_01."/".$MonthStart_01."/01"), 7), "/", "-"));
		}
		$_SESSION["SQL_REP0701"]="select ".$_SESSION["SQL_REP0701"]." pi02 as LeftTitleName from productinfo where (1=1)";
		...
		for ($x=0; $x<$iDiff; $x++) {
			...
			if ($sSeasonMonthStartNow==1) {
				$_SESSION["TitleTdName_REP0701"]=UnidimensionalArrayAppend($_SESSION["TitleTdName_REP0701"], $sLoopYear."-第一季");
				$sSeasonMonthStartNow=2;
				//SQL Script
				$_SESSION["SQL_REP0701"]=$_SESSION["SQL_REP0701"]."(select count(*) from orderdetail where orderdetail.pi01=productinfo.pi01 and (select oh26 from orderheader where orderheader.oh01=orderdetail.oh01) in ('退貨處理中', '已退貨', '已換貨', '已退款') and left(lasttime, 4) in ('".$sLoopYear."-01', '".$sLoopYear."-02', '".$sLoopYear."-03')) as '".$sLoopYear."01', ";
				//Field
				$_SESSION["Field_REP0701"]=UnidimensionalArrayAppend($_SESSION["Field_REP0701"], $sLoopYear."01");
			} else if ($sSeasonMonthStartNow==2) {
				$_SESSION["TitleTdName_REP0701"]=UnidimensionalArrayAppend($_SESSION["TitleTdName_REP0701"], $sLoopYear."-第二季");
				$sSeasonMonthStartNow=3;
				//SQL Script
				$_SESSION["SQL_REP0701"]=$_SESSION["SQL_REP0701"]."(select count(*) from orderdetail where orderdetail.pi01=productinfo.pi01 and (select oh26 from orderheader where orderheader.oh01=orderdetail.oh01) in ('退貨處理中', '已退貨', '已換貨', '已退款') and left(lasttime, 4) in ('".$sLoopYear."-04', '".$sLoopYear."-05', '".$sLoopYear."-06')) as '".$sLoopYear."02', ";
				//Field
				$_SESSION["Field_REP0701"]=UnidimensionalArrayAppend($_SESSION["Field_REP0701"], $sLoopYear."02");
			} else if ($sSeasonMonthStartNow==3) {
				$_SESSION["TitleTdName_REP0701"]=UnidimensionalArrayAppend($_SESSION["TitleTdName_REP0701"], $sLoopYear."-第三季");
				$sSeasonMonthStartNow=4;
				//SQL Script
				$_SESSION["SQL_REP0701"]=$_SESSION["SQL_REP0701"]."(select count(*) from orderdetail where orderdetail.pi01=productinfo.pi01 and (select oh26 from orderheader where orderheader.oh01=orderdetail.oh01) in ('退貨處理中', '已退貨', '已換貨', '已退款') and left(lasttime, 4) in ('".$sLoopYear."-07', '".$sLoopYear."-08', '".$sLoopYear."-09')) as '".$sLoopYear."03', ";
				//Field
				$_SESSION["Field_REP0701"]=UnidimensionalArrayAppend($_SESSION["Field_REP0701"], $sLoopYear."03");
			} else if ($sSeasonMonthStartNow==4) {
				$_SESSION["TitleTdName_REP0701"]=UnidimensionalArrayAppend($_SESSION["TitleTdName_REP0701"], $sLoopYear."-第四季");
				$sSeasonMonthStartNow=1;
				//SQL Script
				$_SESSION["SQL_REP0701"]=$_SESSION["SQL_REP0701"]."(select count(*) from orderdetail where orderdetail.pi01=productinfo.pi01 and (select oh26 from orderheader where orderheader.oh01=orderdetail.oh01) in ('退貨處理中', '已退貨', '已換貨', '已退款') and left(lasttime, 4) in ('".$sLoopYear."-10', '".$sLoopYear."-11', '".$sLoopYear."-12')) as '".$sLoopYear."04', ";
				//Field
				$_SESSION["Field_REP0701"]=UnidimensionalArrayAppend($_SESSION["Field_REP0701"], $sLoopYear."04");
		...
		for ($x=0; $x<$iDiff; $x++) {
			//上方Title
			$_SESSION["TitleTdName_REP0701"]=UnidimensionalArrayAppend($_SESSION["TitleTdName_REP0701"], Replace(Left(DateAdd("yyyy", $x, $YearStart_03."/01/01"), 4), "/", "-"));
			//SQL Script
			$_SESSION["SQL_REP0701"]=$_SESSION["SQL_REP0701"]."(select count(*) from orderdetail where orderdetail.pi01=productinfo.pi01 and (select oh26 from orderheader where orderheader.oh01=orderdetail.oh01) in ('退貨處理中', '已退貨', '已換貨', '已退款') and left(lasttime, 4)='".Replace(Left(DateAdd("yyyy", $x, $YearStart_03."/01/01"), 4), "/", "-")."' group by left(lasttime, 4)) as '".Replace(Left(DateAdd("yyyy", $x, $YearStart_03."/01/01"), 4), "/", "-")."', ";
			//Field
			$_SESSION["Field_REP0701"]=UnidimensionalArrayAppend($_SESSION["Field_REP0701"], Replace(Left(DateAdd("yyyy", $x, $YearStart_03."/01/01"), 4), "/", "-"));
		}
		$_SESSION["SQL_REP0701"]="select ".$_SESSION["SQL_REP0701"]." pi02 as LeftTitleName from productinfo where (1=1)";
	}

	uLog($_SESSION["TitleTdName_REP0701"]);
...
HtmlManTopII();
//HtmlManMenuII();
HtmlManAPNameII("REP0701 - 統計分析 -＞ 退貨數量");
HtmlManContentTopII();

RwForm("", "", "frmUser", "", True);
FormList($sTitleBarF, $aTitleF, $aWidthF, $aItemCodeF, $aItemParameterF, $aDescriptionF, $aButtonF, $aVisableF);
ReportDatalist($_SESSION["SQL_REP0701"], $_SESSION["TitleTdName_REP0701"], $_SESSION["Field_REP0701"]);
RwForm("", "", "", "", False);
